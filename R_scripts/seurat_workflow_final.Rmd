...
# ------------------------------------------------------------------------------
# mouse <- correct_counts_soupx(species = "mouse")
human <- correct_counts_soupx(species = "human", author = "Knafo")
# knafo <- correct_counts_soupx(species = "mouse", author = "Knafo")
```
...
# Example usage
# ------------------------------------------------------------------------------
# mouse_list <- correct_counts_soupx_seurat(species = "mouse_2", author = "Skinnider")
human_list <- correct_counts_soupx_seurat(species = "human", author = "Knafo")
# knafo_list <- correct_counts_soupx_seurat(species = "mouse", author = "Knafo")
```
...
# Example usage
# ------------------------------------------------------------------------------
citeseq_path <- file.path(getwd(), "..", "count_matrices", "mouse_2", "Knafo", "citeseq")
citeseq_list <- correct_citeseq_counts_soupx(citeseq_path = citeseq_path)
...
# ------------------------------------------------------------------------------
# saveRDS(mouse_list, file = file.path(mouse_save_path, "knaf14_soupx.rds"))
saveRDS(human_list, file = file.path(human_save_path, "human_knaf89_soupx.rds"))
```
...
```
```{r}
# Rewriting the paramSweep functoin to be able to modify the range of sweeps
paramSweep <- function (seu, PCs = 1:10, sct = FALSE, num.cores = 1, pN = seq(0.05, 0.3, by = 0.05), pK = c(5e-04, 0.001, 0.005, seq(0.01, 0.3, by = 0.01))) 
{
    require(Seurat)
    require(fields)
    require(parallel)
...
detect_doublets <- function(seurat_list, base_rate = 0.008) {
  ...
  for (i in seq_along(seurat_list)) {
    ...
    # ------------------------------------------------------------------------
    sweep_list  <- paramSweep(seurat_obj, PCs = 1:30, sct = TRUE, pN = seq(0.25, 0.3, by = 0.05))  #!!# Changed pN range
...
# ------------------------------------------------------------------------------
#saveRDS(mouse_doublets, file = file.path(mouse_save_path, "mouse_doublets_knaf4.rds"))
saveRDS(human_doublets, file = file.path(human_save_path, "human_knaf89_doublets.rds"))
```
...
human_doublets <- readRDS(file.path(human_save_path, "human_knaf1415_doublets.rds"))
```
...
plt <- QC_Plots_Combined_Vln(merged_human,  # or merged_human
...
plt
# Save 
ggsave(filename = file.path(human_path_figures,  # or human_path_figures
...
plt_post
ggsave(filename = file.path(human_path_figures, 
...
saveRDS(merged_human, file = file.path(human_save_path, "human_knaf89_merged_filtered.rds"))

# merged_human = readRDS(file = file.path(human_save_path, "human_merged_filtered.rds"))
```
...
# UMAP visualization
# Group by samples
p1 <- apply_theme(DimPlot(merged_human, # or merged_human
                                   reduction = "umap", group.by = "orig.ident"), "Cells distribution per sample - Pre-integration")
p1
# Split by samples
p2 <- apply_theme(DimPlot(merged_human, # or merged_human
...
```{r}
hist(gfp_expression$GFP, breaks = 100)
#merged_mouse <- AddMetaData(merged_mouse, metadata = gfp_expression, col.name = "GFP")
#merged_mouse <- subset(merged_mouse, subset = GFP < 700)
#merged_mouse <- NormalizeADTData(merged_mouse, assay = "Antibody")
```


```{r}
hist(gfp_expression_norm$GFP, breaks = 100)
```
...
```{r}
sum(gfp_expression_norm$GFP < 1)
sum(gfp_expression_norm$GFP > 1)
```


```{r}
clr_gfp <- clr(x = gfp_expression$GFP)
clr_gfp <- as.vector(clr_gfp)
hist(clr_gfp, breaks = 100)
```
```{r}
m <- mean(gfp_expression$GFP)
s <- sd(gfp_expression$GFP)

z_score = (gfp_expression$GFP-m/s)
hist(z_score, breaks = 100)
```
```{r}
sum(z_score < 30)
```
...
annotation <- read_excel('/shared/projects/cross_species_prof_scn/knaf14_annotation.xlsx')
annotation <- data.frame(x = annotation$predicted_annotation, row.names = annotation$...1)
annotation <- cbind(gfp_expression,annotation[rownames(gfp_expression),])
annotation <- cbind(gfp_expression_norm, annotation)
annotation <- cbind(annotation, clr_gfp)
colnames(annotation) <- c('norm_GFP', 'GFP', 'annotation', 'clr_GFP')
annotation
```
```{r}
hist(subset(annotation, subset = annotation == 'Not_V1_predicted')$GFP, breaks = 100, title = 'Not V1 predicted')
hist(subset(annotation, subset = annotation == 'V1_predicted')$GFP, breaks = 100, title = 'V1 predicted')
```
```{r}
hist(subset(annotation, subset = annotation == 'Not_V1_predicted')$norm_GFP, breaks = 100, title = 'Not V1 predicted')
hist(subset(annotation, subset = annotation == 'V1_predicted')$norm_GFP, breaks = 100, title = 'V1 predicted')
```


```{r}
hist(subset(annotation, subset = annotation == 'Not_V1_predicted')$clr_GFP, breaks = 100, title = 'Not V1 predicted')
hist(subset(annotation, subset = annotation == 'V1_predicted')$clr_GFP, breaks = 100, title = 'V1 predicted')
```
...
#-------------------------------------------------------------------------------
# Filter cells from CITEseq sample where GFP < 0.05
#-------------------------------------------------------------------------------
merged_mouse <- subset(merged_mouse, subset = GFP > 0.05 | orig.ident != "KNAF14")
...
# ------------------------------------------------------------------------------
# saveRDS(merged_mouse, file = file.path(mouse_save_path, "all_samples", "mouse_processed_knaf14.rds"))
saveRDS(merged_human, file = file.path(human_save_path, "all_samples", "human_knaf1415_processed.rds"))
```
...
```{r}
# Load the RDS file
mouse_doublets <- readRDS(file.path(mouse_save_path, "all_samples", "mouse_doublets.rds"))
```
...
```{r}
merged_mouse <- readRDS(file.path(getwd(), "Rdata/mouse/all_cells", "all_samples", "mouse_integrated_skinnider.rds"))
```
```{r}
merged_mouse <- subset(merged_mouse, subset = orig.ident %in% c("GSE7474501","GSM7474503"))
```
...
```{r}
# UMAP visualization
plt <- apply_theme(DimPlot_scCustom(merged_human, # or merged_human
...
# ------------------------------------------------------------------------------
saveRDS(merged_mouse, file = file.path(mouse_save_path, "all_samples", "mouse_integrated_skinnider_only2.rds"))
...
```{r}
# Load the RDS files
# merged_mouse <- readRDS(file.path(mouse_save_path, "all_samples", "mouse_integrated_skinnider_only2.rds"))
...
#-------------------------------------------------------------------------------
plt <- DotPlot(neurons_mouse, # or merged_human
...
ggsave(filename = file.path(mouse_path_figures, # or human_path_figures
                            "all_samples", "dotplot_mouse_neurons.jpg"), plot = plt, width = 16, height = 8)
```
...
# Example Usage
#-------------------------------------------------------------------------------
neurons_mouse <- annotate_celltypes(neurons_mouse, celltype_markers = celltype_markers)
...
```{r}
# UMAP visualization 
plt <- apply_theme(DimPlot(neurons_mouse, # or merged_human
...
ggsave(filename = file.path(mouse_path_figures, # or human_path_figures
                       "all_samples",
                       "cell_types_neuronsonly.png"), plot = plt, width = 10, height = 8)
```

```{r}
plt <- FeaturePlot(neurons_mouse, features = c('Slc17a6'))
plt
```
...
summarize_celltypes <- function(seurat_object) {
  ...
  for (ori in unique(seurat_object$orig.ident)){
    # Create a table to count the number of cells in each cell type
    celltype_counts <- table(subset(seurat_object, subset = orig.ident %in% c(ori))@meta.data$celltype)
...
# ------------------------------------------------------------------------------
saveRDS(merged_mouse, file = file.path(mouse_save_path, "all_samples", "mouse_annotated_skinnider_only2.rds"))
...
Idents(neurons_mouse) <- "celltype"
# Idents(merged_human) <- "celltype"

# Create a subset of neurons only 
neurons_mouse <- subset(neurons_mouse, idents = "neurons")
...
```{r}
# Save 
saveRDS(neurons_mouse, file = file.path(mouse_save_path, "all_samples", "mouse_neurons_skinnider.rds"))
